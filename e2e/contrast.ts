 ...
 *    files the node under `incomplete`. The `.select` object-format control
 *    paints its own chevron from two `linear-gradient` layers, and every tinted
 *    surface on this page is a `color-mix(..., transparent)` wash rather than a
 *    flat fill: `.commit-row.is-selected`, `.file-tab.is-active`, `.is-repo-row`,
 *    `.oid`, `.bar-track` and the `.reveal` panel all composite onto whatever is
 *    behind them rather than onto a colour axe can read off one declaration.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. This sheet deliberately
 *    de-emphasises with a dimmer *colour* rather than opacity, but `.contrib-row`
 *    animates from `opacity: 0` under `prefers-reduced-motion: no-preference`,
 *    so an opacity-blind oracle would still be reading a colour the screen does
 *    not show while the entropy table staggers in.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. This page is built
 *    out of scrollers: `.commit-scroll`, `.code-block`, `.object-scroll`,
 *    `.contrib-scroll`, `.finding-code`, `.verify-block` and every `.table-x`
 *    wrapper. At 380px the `.entropy-table`, `.remedy-table` and `.rules-table`
 *    each carry `min-width: 34rem`, so most of their width is deliberately
 *    clipped away inside `.table-x`. Content scrolled past the client box is not
 *    dimmed or partly drawn — it is absent from the frame, and asking what colour
 *    it sits on has no answer. Its rect is still to the right of every ancestor's
 *    box, so the ancestor walk would find nothing behind it and fall through to
 *    white, inventing a failure. Skip it, and rely on the wider viewport where
 *    the very same element is visible and measured for real.
 *
 ...
 *    two lines and silent when unused, and it is what stops a future
 *    gradient-clipped heading from being reported as a 1:1 failure.
 *
 *  - SVG PAINTS IN DOCUMENT ORDER, SO SIBLINGS CAN BE THE BACKGROUND. This lab's
 *    only SVG is the shared top bar's icon set, which carries no `<text>` at all,
 *    so the rule never fires here — it is kept because it is cheap and silent
 *    when unused: an `<svg>` draws a shape as a `<rect>` and then its label as a
 *    following `<text>`, so a label's backdrop is a PRECEDING SIBLING shape, not
 *    an ancestor. An ancestor-only walk would composite those labels onto the
 *    svg's own transparent background and report a ratio nothing on screen has.
 *    So for SVG text, any earlier sibling shape whose box contains the text box
 *    is composited first.
 */
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     * accent into surfaces with `color-mix()` in `oklab` throughout (the hero's
     * "why it matters" wash, the selected commit row, the active file tab, the
     * `.reveal` alarm panel), the shared top bar mixes in `srgb`, and Chromium
     * reports every one of those to `getComputedStyle` unchanged rather than
     * converting them to sRGB. A regex that only understands `rgb()/rgba()` sees
     * `null` for every one of them and the walk then falls through to the wrong
     * backdrop — which fabricates failures (a selected commit row's light ink
     * read as sitting on the page's dark canvas rather than on its own accent
     * wash) and, far worse, could hide a real one. The 2D canvas is the browser's own colour
    ...
    const resolve = (c: string): RGBA | null => {
      ...
      if (fromBlack === fromWhite) {
        ...
        // Tuple-typed: this repo compiles the e2e tree under
        // `noUncheckedIndexedAccess`, which types every `Uint8ClampedArray`
        // index as `number | undefined`. The four channels are guaranteed by
        // the 1x1 read.
        const d = ctx.getImageData(0, 0, 1, 1).data as unknown as [
          number,
          number,
          number,
          number,
    ...
    const colorAt = (stops: Stop[], t: number): RGBA => {
      ...
      let i = 0;
      while (i < stops.length - 1 && (stops[i + 1] as Stop).pos < t) i++;
    ...
    const parseStops = (parts: string[]): Stop[] => {
      type RawStop = { color: RGBA; pos: number | null };
      const raw: RawStop[] = [];
      ...
      if (!head || !tail) return [];
      if (head.pos === null) head.pos = 0;
      if (tail.pos === null) tail.pos = 1;
      for (let i = 1; i < raw.length - 1; i++) {
        if ((raw[i] as RawStop).pos !== null) continue;
        let j = i;
        while (j < raw.length && (raw[j] as RawStop).pos === null) j++;
        const lo = (raw[i - 1] as RawStop).pos as number;
        const hi = (raw[j]?.pos as number) ?? 1;
        for (let k = i; k < j; k++) {
          (raw[k] as RawStop).pos = lo + ((hi - lo) * (k - i + 1)) / (j - i + 1);
        }
      }
     ...
     * for a gradient that spans its element and wrong for a small one anchored to
     * a corner — which is exactly what `.select` has: two 5x5px `linear-gradient`
     * layers pinned near the right edge to draw the chevron, over a
     * `background-color` that covers the rest. Judged at its worst stop, the
     * option text would be measured against a `currentColor` fill that occupies
     * ten pixels of a 26rem control. So each gradient is *sampled* at the text's
    ...
    const paintAt = (cs: CSSStyleDeclaration, rect: DOMRect, p: Point): RGBA => {
      ...
      for (let i = layers.length - 1; i >= 0; i--) {
        result = over(sampleLayer(layers[i] as string, rect, p), result);
      }
      return result;
    };
     ...
     *
     * The per-character contribution table renders one row per distinct
     * character of the scored string, the object-store table one row per git
     * object, and the entropy chart one row per sample plus every candidate the
     * repository yielded — hundreds of text nodes, all walking the same handful
     * of panel ancestors. Without the caches the pass re-reads the same computed
     * styles and rects tens of thousands of times. Nothing mutates the DOM during
     ...
     * This page uses the classic recipe verbatim: `.visually-hidden` is
     * `position: absolute; width: 1px; height: 1px; clip: rect(0 0 0 0)`, and the
     * per-character entropy table emits one of them per row to spell out the
     * character name ("space", "newline") beside the `aria-hidden` glyph. Without
     * this guard the walk composites all of them against whatever surface they
     * happen to sit on and INVENTS a failure per row for text nobody can see.
     * Deliberately narrow — only a ZERO-area clip qualifies, so a real partial
     * clip is still measured.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This page has five `.disclose` panels (glossary, hash
      // verification, the object store, the format-rule table, the threshold
      // provenance); the gate opens each by clicking its summary and scans the
      // open state for real rather than trusting the closed geometry.
      ...
      // header's `.cl-skip-link` parks at `top: -3rem` and slides to `top: 0`
      // only on focus. Measuring the parked copy invents a failure for text that
      // is not on screen; the focused rendering is a real state and the gate
      // scans it explicitly instead.
     ...
     * stricter than axe on decorative content. Honour the same boundary — this
     * page's `.chip-icon` glyphs (✕ ✓ ! · ○) are `aria-hidden` twins of the chip
     * label right beside them, and the `.char-cell` glyphs in the contribution
     * table are paired with a `.visually-hidden` character name.
     *
     * BUT NOTE THE COST, because it is a real blind spot and this lab has one:
     * `aria-hidden` removes text from the accessibility tree, not from the
     * screen. WCAG 1.4.3 still applies to anything a sighted reader can read. The
     * `.line-no` gutter in `.code-block` is `aria-hidden` and visible, so BOTH
     * oracles skip it — it was measured by hand instead (see the 1.4.3 note in
     * `style.css`) and it was a genuine failure at 3.96:1.
     */
     ...
     * `html { background-color: var(--bg) }` and runs several viewports tall, so
     * the canvas is doing real work: text in the footer or the fix panel is far
     * below the root's own box. A lab that sets `html, body { height: 100% }` on
     * a tall document has both boxes exactly one viewport tall, every element
     * below the fold intersects neither, the walk ends transparent, and it falls
     * through to WHITE — reporting dark-theme text against a page that does not
     * exist. In one lab that was 34 of 38 findings in a single run, and it can
     * mask a real failure in the other direction just as easily.
     *
